
-- Staged ESPN match enrichment data.
-- Cleans types and provides a stable interface for gold layer joins.
-- Returns empty result set if bronze.espn_matches doesn't exist yet (enrichment not run).
{% if source_exists('bronze', 'espn_matches') %}
select
    cricsheet_match_id as match_id,
    espn_match_id,
    espn_series_id,
    floodlit,
    cast(start_time as timestamp) as start_time,
    cast(end_time as timestamp) as end_time,
    hours_info,
    -- Classification
    international_class_id,
    sub_class_id,
    -- Venue enrichment
    espn_ground_id,
    -- Clean ground_capacity: strip commas and non-numeric suffixes (e.g. "60,000 approx" → 60000)
    try_cast(
        regexp_replace(replace(ground_capacity, ',', ''), '[^0-9].*', '')
        as integer
    ) as ground_capacity,
    venue_timezone,
    -- Team 1
    team1_name,
    team1_long_name,
    team1_espn_id,
    team1_captain,
    team1_keeper,
    team1_is_home,
    team1_points,
    team1_primary_color,
    team1_abbreviation,
    -- Team 2
    team2_name,
    team2_long_name,
    team2_espn_id,
    team2_captain,
    team2_keeper,
    team2_is_home,
    team2_points,
    team2_primary_color,
    team2_abbreviation,
    -- Ground country
    ground_name,
    ground_long_name,
    ground_country_name,
    ground_country_abbreviation,
    -- MVP (ESPN's "Most Valued Player of the Match" smart metrics)
    mvp_player_name,
    mvp_smart_runs,
    mvp_smart_wickets,
    mvp_batting_impact,
    mvp_bowling_impact,
    mvp_total_impact,
    -- JSON blobs (kept for downstream extraction into dedicated silver models)
    replacement_players_json,
    debut_players_json,
    teams_enrichment_json,
    player_of_match_json,
    -- Audit
    current_timestamp as _loaded_at

from {{ source('bronze', 'espn_matches') }}
{% else %}
-- Source table does not exist yet — return empty with correct schema
select
    null::varchar as match_id,
    null::bigint as espn_match_id,
    null::bigint as espn_series_id,
    null::varchar as floodlit,
    null::timestamp as start_time,
    null::timestamp as end_time,
    null::varchar as hours_info,
    null::bigint as international_class_id,
    null::bigint as sub_class_id,
    null::bigint as espn_ground_id,
    null::integer as ground_capacity,
    null::varchar as venue_timezone,
    null::varchar as team1_name,
    null::varchar as team1_long_name,
    null::bigint as team1_espn_id,
    null::varchar as team1_captain,
    null::varchar as team1_keeper,
    null::boolean as team1_is_home,
    null::double as team1_points,
    null::varchar as team1_primary_color,
    null::varchar as team1_abbreviation,
    null::varchar as team2_name,
    null::varchar as team2_long_name,
    null::bigint as team2_espn_id,
    null::varchar as team2_captain,
    null::varchar as team2_keeper,
    null::boolean as team2_is_home,
    null::double as team2_points,
    null::varchar as team2_primary_color,
    null::varchar as team2_abbreviation,
    null::varchar as ground_name,
    null::varchar as ground_long_name,
    null::varchar as ground_country_name,
    null::varchar as ground_country_abbreviation,
    null::varchar as mvp_player_name,
    null::double as mvp_smart_runs,
    null::double as mvp_smart_wickets,
    null::double as mvp_batting_impact,
    null::double as mvp_bowling_impact,
    null::double as mvp_total_impact,
    null::varchar as replacement_players_json,
    null::varchar as debut_players_json,
    null::varchar as teams_enrichment_json,
    null::varchar as player_of_match_json,
    current_timestamp as _loaded_at
where false
{% endif %}
