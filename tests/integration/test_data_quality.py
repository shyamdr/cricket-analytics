...
class TestGoldLayerDataIntegrity:
    ...
    def test_no_null_match_ids(self, db_conn) -> None:
        (count,) = db_conn.execute(
            "SELECT COUNT(*) FROM main_gold.dim_matches WHERE match_id IS NULL"
        ).fetchone()
        assert count == 0

    def test_unique_match_ids(self, db_conn) -> None:
        (total,) = db_conn.execute("SELECT COUNT(*) FROM main_gold.dim_matches").fetchone()
        (distinct,) = db_conn.execute(
            "SELECT COUNT(DISTINCT match_id) FROM main_gold.dim_matches"
        ).fetchone()
    ...
    def test_seasons_are_valid(self, db_conn) -> None:
        ...
        seasons = [
            ...
            for r in db_conn.execute(
                "SELECT DISTINCT season FROM main_gold.dim_matches ORDER BY season"
            ).fetchall()
        ]
        for s in seasons:
    ...
    def test_no_2020_2021_season_merge(self, db_conn) -> None:
        ...
        seasons = [
            r[0]
            for r in db_conn.execute("SELECT DISTINCT season FROM main_gold.dim_matches").fetchall()
        ]
        ...
        (count_2020,) = db_conn.execute(
            "SELECT COUNT(*) FROM main_gold.dim_matches WHERE season = '2020'"
        ).fetchone()
        (count_2021,) = db_conn.execute(
            "SELECT COUNT(*) FROM main_gold.dim_matches WHERE season = '2021'"
        ).fetchone()
    ...
    def test_match_dates_in_range(self, db_conn) -> None:
        ...
        min_date, max_date = db_conn.execute(
            "SELECT MIN(match_date), MAX(match_date) FROM main_gold.dim_matches"
        ).fetchone()
    ...
    def test_every_match_has_two_teams(self, db_conn) -> None:
        (count,) = db_conn.execute(
            "SELECT COUNT(*) FROM main_gold.dim_matches " "WHERE team1 IS NULL OR team2 IS NULL"
        ).fetchone()
    ...
    def test_batting_innings_reference_valid_matches(self, db_conn) -> None:
        ...
        (orphans,) = db_conn.execute("""
            SELECT COUNT(*) FROM main_gold.fact_batting_innings bi
            LEFT JOIN main_gold.dim_matches m ON bi.match_id = m.match_id
    ...
    def test_bowling_innings_reference_valid_matches(self, db_conn) -> None:
        (orphans,) = db_conn.execute("""
            SELECT COUNT(*) FROM main_gold.fact_bowling_innings bo
            LEFT JOIN main_gold.dim_matches m ON bo.match_id = m.match_id
            WHERE m.match_id IS NULL
        """).fetchone()
    ...
    def test_no_negative_runs(self, db_conn) -> None:
        (count,) = db_conn.execute(
            "SELECT COUNT(*) FROM main_gold.fact_batting_innings WHERE runs_scored < 0"
        ).fetchone()
        assert count == 0
    ...
    def test_strike_rate_reasonable(self, db_conn) -> None:
        ...
        (count,) = db_conn.execute(
            "SELECT COUNT(*) FROM main_gold.fact_batting_innings "
    ...
    def test_economy_rate_reasonable(self, db_conn) -> None:
        ...
        (count,) = db_conn.execute(
            "SELECT COUNT(*) FROM main_gold.fact_bowling_innings " "WHERE economy_rate < 0"
        ).fetchone()
